...
function connect(){
  ...
  ws.onmessage=e=>{let m;try{m=JSON.parse(e.data);}catch{return;}if(m.v!==VERSION)return;if(m.type==='ready'){
    clearTimeout(authTimer);state.connected();status='Connected';lastPong=Date.now();docs.clear();send({v:VERSION,type:'pin',tab:config.pin??null});void fresh();
  ...
  ws.onclose=e=>{if(socket!==ws)return;socket=null;state.disconnected();clearTimeout(authTimer);clearInterval(heartbeat);status=e.code===4003?'Pairing rejected. Copy fresh details from the desktop.':'Disconnected — retrying';if(e.code!==4003)reconnect=setTimeout(connect,state.nextDelay());};
}
async function restart(){clearTimeout(reconnect);clearTimeout(authTimer);clearInterval(heartbeat);const old=socket;socket=null;old?.close();state.disconnected();connect();}
chrome.runtime.onMessage.addListener((m,sender,reply)=>{
  ...
  void initialized.then(async()=>{
    if(m?.type==='status')reply({status,port:config.port??DEFAULT_PORT,paired:!!config.secret,pin:config.pin??null});
...
chrome.tabs.onUpdated.addListener((tab,change)=>{if(!change.url)return;if(sourceUrl(change.url)){tabUrls.set(tab,change.url);void chrome.tabs.sendMessage(tab,{type:'fresh'}).catch(()=>{});}else{tabUrls.delete(tab);docs.delete(tab);send({v:VERSION,type:'remove',tab});}});
chrome.tabs.onRemoved.addListener(tab=>{tabUrls.delete(tab);docs.delete(tab);send({v:VERSION,type:'remove',tab});if(config.pin===tab){config.pin=null;void chrome.storage.session.set({pin:null});}});
