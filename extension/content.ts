import { getVideo, watchId, readMetadata, adState, observeChanges,rememberContent,forgetContent,metadataEvidence } from './adapter';
...
function send(){
  ...
  if(ad==='content')Object.assign(p,metadataEvidence(id));
  lastSent=now;void chrome.runtime.sendMessage({type:'playback',document:documentKey,playback:p}).catch(()=>{});return p;
}
...
chrome.runtime.onMessage.addListener((m,_sender,reply)=>{if(m?.type==='fresh'){send();reply({ok:!stopped});}else if(m?.type==='capture-anchor'){const playback=send();reply({playback,capturedAt:Date.now()});}});
function start(){
