...
import { settings,recoverBounds,resolvedTheme,shouldShowOverlay } from '../shared/settings';
...
function syncVisibility(){if(!overlay||!controller||!overlayReady)return;const source=controller.sessions.current();const playing=!!source&&source.playback.playing&&!source.playback.ended&&performance.now()-source.received<STALE_MS;const visible=shouldShowOverlay(store.data.settings,playing);if(visible!==overlay.isVisible()){if(visible)overlay.showInactive();else overlay.hide();}}
...
function showSettings(section:'general'|'match'='general'){settingsNavigation={section,request:settingsNavigation.request+1};broadcast();if(panel){panel.setTitle(section==='match'?'LyricGlass • Choose lyrics':'LyricGlass • Settings');panel.show();panel.focus();return;}panel=new BrowserWindow({width:880,height:780,minWidth:780,minHeight:700,resizable:true,frame:false,show:false,title:section==='match'?'LyricGlass • Choose lyrics':'LyricGlass • Settings',backgroundColor:'#141418',autoHideMenuBar:true,webPreferences:{preload:path.join(__dirname,'preload.cjs'),contextIsolation:true,sandbox:true,nodeIntegration:false,webSecurity:true}});secureWindow(panel);const settingsWindow=panel;settingsWindow.once('ready-to-show',()=>{if(!settingsWindow.isDestroyed()){settingsWindow.show();settingsWindow.focus();}});panel.on('maximize',broadcast);panel.on('unmaximize',broadcast);panel.on('page-title-updated',event=>event.preventDefault());void settingsWindow.loadFile(index,{hash:'settings'}).then(()=>{if(!settingsWindow.isDestroyed()){settingsWindow.show();settingsWindow.focus();}});panel.on('closed',()=>panel=null);}
...
async function command(e:Electron.IpcMainInvokeEvent,c:unknown){
  ...
  try{
    ...
    switch(c.type){
      ...
      case 'settings':{
        if(!object(c.patch)||Object.keys(c.patch).some(k=>!(isPanel?['port','opacity','textOnly','fontSize','width','theme','launchAtStartup','onlyWhilePlaying','karaoke']:['theme','opacity']).includes(k)))throw Error('Invalid settings');
        ...
        if(next.launchAtStartup!==store.data.settings.launchAtStartup)configureStartup(app,next.launchAtStartup,app.isPackaged,process.execPath,app.getAppPath());
