...
describe('bridge with mocked sockets and HTTP listener',()=>{
 ...
 it('pins origin on successful auth, acknowledges heartbeats and rejects malformed data',()=>{const{bridge,store,messages}=setup();const ws=new Socket();connect(ws);ws.message(auth);expect(store.data.origin).toBe(origin);expect(ws.sent).toEqual([{v:1,type:'ready',captureVersion:1}]);ws.message({v:1,type:'heartbeat'});expect(ws.sent.at(-1).type).toBe('pong');ws.message({v:1,type:'snapshot',tab:1,document:'document1',playback:playback()});expect(messages).toHaveLength(1);ws.message('{bad');expect(ws.code).toBe(4002);bridge.stop();});
...
it('keeps source-isolated sample transport behind the existing authentication boundary',()=>{const{bridge,messages}=setup();const ws=new Socket();connect(ws);ws.message({v:1,type:'capture-state',tab:1,active:true});expect(ws.code).toBe(4003);expect(messages).toHaveLength(0);expect(bridge.send('unknown',{type:'capture-request'})).toBe(false);bridge.stop();});
